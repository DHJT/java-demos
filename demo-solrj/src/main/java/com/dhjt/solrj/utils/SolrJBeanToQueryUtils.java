...
/**
 * Slor查询语句拼接工具：参考http://www.2cto.com/kf/201511/451412.html
 *
 * @packge com.boonya.solr.utils.SolrJBeanToQueryUtils
 * @date   2017年2月26日  下午10:09:16
 * @author pengjunlin
 * @comment   查询（查询语句构造和执行查询，分页查询），更新，重建索引
 * @update
 */
public class SolrJBeanToQueryUtils<T> {

	protected final Logger logger = LoggerFactory
			.getLogger(SolrJBeanToQueryUtils.class);

	private SolrClient server;

	private StringBuffer queryString;

	public SolrJBeanToQueryUtils(String reqUrl) {
		server = new HttpSolrClient.Builder(reqUrl).build();
	...
	private static void in(StringBuffer queryString, String fieldName,
			List<String> vals) {
		queryString.append("(");
	...
	public List<T> query(String sort, Class<T> beanClass) {
		...
		try {
			...
			response = server.query(params);
			list = response.getBeans(beanClass);
		} catch (SolrServerException e) {
	...
	public Page<T> execQuery(Integer pageNo, Integer rows, String sort,
		...
		try {
			...
			response = server.query(params);
			results = response.getBeans(beanClass);
